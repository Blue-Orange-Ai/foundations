...
|---|---|---|
| `ci.yml` | push / PR to `main` | `lerna run build` + `lerna run test`, with lockfile-drift recovery (see below) |
...
`npm ci` refuses to run when `package-lock.json` does not match the
`package.json` manifests — a dependency bumped without regenerating the lock,
or a lockfile regenerated by someone (usually a dependency bot) without
credentials for the private `primitives-*` packages. That used to kill every
workflow at the install step, before a single package was built, and the runs
that followed on `main` stayed red until someone with registry access
regenerated the lockfile by hand.

`ci.yml` now recovers instead of dying:

1. `npm ci` runs first. If it fails, CI falls back to a fresh `npm install`
   so the **full** build and test signal is still produced.
2. The regenerated, in-sync `package-lock.json` is uploaded as the
   `synced-package-lock` artifact on every drifted run.
3. On a same-repo pull request, CI regenerates the lockfile against the PR's
   own branch (in a worktree of the PR head, so re-runs produce no diff and
   commit loops are impossible) and pushes it as
   `chore(ci): sync package-lock.json with package.json`. That push is made
   with `PAT_TOKEN`, so it triggers a fresh CI run that installs cleanly —
   dependency-bot PRs heal themselves.
4. The drifted run itself is still **failed**, with an error annotation and a
   step summary pointing at the fix. A green check on a stale lockfile would
   let the drift merge and break `npm ci` on `main` and in every release
   workflow.

`versioning.yml` and `maintenance-release.yml` are part of the same story:
`lerna version` ends by running `npm install --package-lock-only`, which
re-resolves against the registry whenever the lockfile has drifted — including
the private `primitives-*` packages. Those steps now run with
`NODE_AUTH_TOKEN`, so cutting a release re-syncs the lockfile in the release
commit instead of dying with a 401 (which is what blocked v0.4.11).
`dirty-build.yml` falls back to `npm install` on drift — a dirty build is a
throwaway prerelease and should not be blocked by a stale lockfile.

To fix drift by hand, run `npm install` at the repo root with
`npm.pkg.github.com` credentials (see below) and commit the resulting
`package-lock.json` — or just download the `synced-package-lock` artifact from
the failed run.
...
- `versioning.yml` and `maintenance-release.yml` run Lerna through
  `npx lerna@8.2.4` (pinned to the root devDependency) instead of installing the
