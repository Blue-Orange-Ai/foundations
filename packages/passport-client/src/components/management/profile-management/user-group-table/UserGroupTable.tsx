...
interface Props {
	...
	size?: number,
	changeRequest?: Date;
	onSelectedGroups?: (selectedGroups: Array<GroupSelectionState>) => void;
}
...
export const UserGroupTable: React.FC<Props> = ({
	...
	const [latestSelected, setLatestSelected] = useState<string | undefined>(undefined);
	...
	const getGroupsBetweenLatestSelected = (groupId: string) => {
		if (latestSelected === undefined) {
			return [groupId];
		}
		var toggleGroups: Array<string> = [];
		var found = false;
		var finished = false;
		for (let group of groups) {
	...
	const toggleSelectionStates = (groupIds: Array<string>) => {
	...
	const toggleSelectionState = (groupId:string, event: React.MouseEvent<HTMLElement>) => {
		event.preventDefault();
		if (event.shiftKey) {
	...
	const getSelectedGroups = (groupSelectionStates: Array<GroupSelectionState>) => {
		var selectedGroups: GroupSelectionState[] = [];
		for (let selectionState of groupSelectionStates) {
			if (selectionState.state) {
				selectedGroups.push(selectionState);
			}
		}
				...
				<div>
					<div className="passport-user-group-table">
						<table className="passport-user-group-table-elem">
							<thead>
							<tr>
								<th onClick={changeGlobalSelectionState}><Checkbox checked={getGlobalGroupSelectionState()} readonly={true} update={forceCheckBoxRender}/></th>
								<th>Group Name</th>
							</tr>
							</thead>
							<tbody>
					...
					<div className="passport-user-group-footer">
						...
						<div className="passport-user-group-footer-right">
							...
							<div className="passport-user-group-table-footer-prev-next-btn">
								<ButtonIcon icon="ri-arrow-left-s-fill" label="Previous Page" isDisabled={workingPage <= 0}></ButtonIcon>
							</div>
							<div className="passport-user-group-table-footer-prev-next-btn">
								<ButtonIcon icon="ri-arrow-right-s-fill" label="Next Page" isDisabled={workingPage + 1 >= Math.ceil(count / workingSize)}></ButtonIcon>
							</div>
						</div>
					</div>
				</div>
